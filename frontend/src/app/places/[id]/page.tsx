      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <section className="mb-10">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {place.imageUrls.slice(1, 5).map((imageUrl, index) => (
                <div
                  key={imageUrl}
                  className={`group overflow-hidden rounded-2xl bg-ink ${index === 0 ? "col-span-2 row-span-2 md:col-span-2" : ""}`}
                >
                  <img
                    src={imageUrl}
                    alt={`${place.name} view ${index + 2}`}
                    loading="lazy"
                    className={`w-full object-cover opacity-90 transition duration-500 group-hover:scale-105 group-hover:opacity-100 ${index === 0 ? "h-full min-h-64" : "h-40 md:h-48"}`}
                  />
                </div>
              ))}
            </div>
        ...
        <div className="mb-10 grid grid-cols-2 gap-3 md:grid-cols-4">
          <div className="rounded-2xl border border-border bg-white p-5">
            ...
            <p className="mt-2 font-serif text-2xl text-ink">
              {place.rating ? `${place.rating.toFixed(1)} / 5` : "New"}
            </p>
          ...
          <div className="rounded-2xl border border-border bg-white p-5">
            ...
            <p className="mt-2 font-serif text-2xl text-ink">
              {place.priceLevel ? "$".repeat(place.priceLevel) : "Not listed"}
            </p>
            <p className="mt-1 text-xs text-muted">Local range</p>
          </div>
          <div className="rounded-2xl border border-border bg-white p-5">
            <p className="text-xs uppercase tracking-wider text-muted">
              Destination
            </p>
            <p className="mt-2 font-serif text-2xl text-ink">{place.city}</p>
            <p className="mt-1 text-xs text-muted">{place.country}</p>
          </div>
          <div className="rounded-2xl border border-border bg-white p-5">
            <p className="text-xs uppercase tracking-wider text-muted">
              Booking
            </p>
            <p className="mt-2 font-serif text-2xl text-ink">Request</p>
            <p className="mt-1 text-xs text-muted">Confirmation required</p>
          </div>
        </div>
        {/* Header */}
        <div className="mb-8">
        ...
        <div className="mb-12 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="overflow-hidden rounded-3xl border border-border bg-white p-8 shadow-sm">
            ...
            {place.amenities?.highlights?.length ? (
              <div className="mt-7">
                <p className="text-xs font-semibold uppercase tracking-wider text-muted">
                  Highlights
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {place.amenities.highlights.map((item) => (
                    <span
                      key={item}
                      className="rounded-full bg-terracotta-tint px-4 py-2 text-sm font-medium text-terracotta"
                    >
                      {item}
                    </span>
                  ))}
            ...
            {place.amenities?.facilities?.length ? (
              <div className="mt-7 grid gap-3 sm:grid-cols-3">
                {place.amenities.facilities.map((item) => (
                  <div
                    key={item}
                    className="rounded-xl border border-border bg-surface p-4 text-sm font-medium text-ink"
                  >
